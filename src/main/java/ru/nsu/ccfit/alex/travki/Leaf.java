...
public class Leaf extends AppCompatActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        form_one = getFilterListElems(getID(l_type[1]));
        one_lst = new ArrayList<>(Arrays.asList(form_one));

        form_two = getFilterListElems(getID(l_type[2]));
        ...
        adapt3 = new ArrayAdapter<>(this, R.layout.row, R.id.elname2, cur_lst);
    ...
    private int getID(String _name){
        Cursor c = dbHelper.mquery("leaf_type", new String[] {"_id"}, "name = ?" , new String[] {_name}, null, null, null);
        if(c.moveToFirst())
            return c.getInt(0);
        return -1;
    }
